<script lang="ts">
    ...
    import { s_eventListStyle } from "../core/eventList/s_eventListStyle";
...
<li class={odd && $s_eventListStyle === "compact" ? "odd" : ""}>
    {#if $s_eventListStyle === "compact"}
        <a
            href="/app/project/{event.project}/event/{event.key}"
            class="compact">
            <div class="title">
...
<style lang="postcss">
    ...
    li a.compact .title {
        @apply flex shrink items-center gap-4 truncate;
    }
    li a.compact .title .icon {
        @apply text-2xl;
    }
    li a.compact .title .title {
        @apply shrink truncate font-medium;
    }
    li a.compact .meta {
        @apply flex w-fit shrink-0 items-center gap-4;
    }

    li a.card {
        @apply mt-6 flex w-full flex-1 gap-4;
        @apply rounded-2xl bg-neutral-50 p-4;
    }
    ...
    li a.card .content .title {
        @apply shrink-0 truncate text-lg font-medium;
    }
    li a.card footer {
