...
{#if $storeSidebarOpen}
    <div 
        ...
        transition:fade="{{ duration: TRANSITION_TOKENS.DURATION + 100, easing: TRANSITION_TOKENS.EASING  }}">
    </div>
{/if}
{#if $storeSidebarOpen}
    <div class:open={$storeSidebarOpen} class="app-sidebar-wrapper"
...
<style lang="postcss">
    .app-sidebar-wrapper {
        @apply isolate h-full max-h-full flex;
    }
    .sidebar-bg {
        @apply hidden;
    }
    aside {
        @apply block max-h-full h-full z-50 bg-white;
        grid-row: 1;
        grid-column: 1;
    }
