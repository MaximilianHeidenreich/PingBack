...
<dialog 
    ...
    <button class="closeBtn" on:click={() => { dialog.close(); }}>
        <IconX size={26} stroke={2}/>
    </button>

    <header class="text-2xl font-semibold mb-6">
        <slot name="title" />
        <p class="text-base font-normal text-neutral-500 mt-2 max-w-[45ch]">
            <slot name="subtitle" />
        </p>
    </header>
    <slot name="body"/>
...
<style lang="postcss">
    dialog {
        @apply px-8 py-6 block bg-white rounded-xl shadow-xl;
    }
    .closeBtn {
