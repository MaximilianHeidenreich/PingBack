...
export async function singleFetchEvents(
    query?: unknown,
    limit?: number,
    lastKey?: string
): Promise<{
    ok: boolean;
    data?: { count: number; last?: string; items: IEvent[] };
    error?: unknown;
}> {
...
export async function* fetchEvents(
    limit: number = 20,
    query: unknown,
    useFetch?: (input: RequestInfo | URL, init?: RequestInit | undefined) => Promise<Response>
) {
    let last = undefined;
    const request = async (
        lastKey: string | undefined,
        query: unknown | undefined
    ): Promise<{ count: number; last: string | undefined; items: IEvent[] }> => {
